...
# Изменения:    добавлено больше комментариев
#               неправильный счёт при столкновении лоб-в-лоб
    ...
    for o in objects:
        ...
        if (o.rect, '1') in tails or (o.rect, '2') in tails \
            ...
            if (time.time() - check_time) >= 0.1:
                check_time = time.time()
                if objects[0].x != objects[1].x and objects[0].y != objects[1].y:
                    if o.color == P1_COLOR:
                        players_score[1] += 1
                    else:
                        players_score[0] += 1
